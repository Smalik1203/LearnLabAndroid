    ...
    Column(
        ...
        Column(
            ...
            SettingsSection(title = "Appearance") {
                SettingRow(label = "Theme", description = "Background and colour palette") {
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ThemeChip(label = "Dark",  selected = isDark == "dark",  onClick = { if (isDark != "dark")  state.toggleTheme() })
                        ThemeChip(label = "Light", selected = isDark == "light", onClick = { if (isDark != "light") state.toggleTheme() })
                    }
                }
                SettingDivider()
                SettingRow(label = "Text Size", description = "Scales all text in the app") {
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        fontScaleOptions.forEach { (scale, label, _) ->
                            FontScaleChip(label = label, selected = fontScale == scale, onClick = { state.setFontScale(scale) })
                        }
                    }
                }
            }
            ...
            SettingsSection(title = "Classroom") {
                SettingRow(label = "Keep Screen On", description = "Prevent the tablet from sleeping during a lesson") {
                    Switch(
                        checked         = keepScreen,
                        onCheckedChange = { state.setKeepScreenOn(it) },
                        colors          = SwitchDefaults.colors(
                            checkedThumbColor   = NavyDeep,   checkedTrackColor   = CyanBright,
                            uncheckedTrackColor = SurfaceMid, uncheckedThumbColor = OnSurfaceLow,
                        ),
                    )
                }
                SettingDivider()
                SettingRow(label = "Fullscreen", description = "Hide the system status and navigation bars") {
                    Switch(
